...
public class formMenuController implements Initializable{
	...
	// Event Listener on Button[#fullscreen].onAction
	    ...
	    public void initialize(URL location, ResourceBundle resources) {
			...
			if(e.getFirst()==""){
				  con.loadAnchorPane(paneData, "plateforme.fxml");
				
			}
	    	//if(pp.get)Employe findEmployeefirt(String ecole,int id) 
	        rec2 = Screen.getPrimary().getVisualBounds();
	        listMenu.getItems().addAll("  Employee management", "  Student management", "  University management", "  Profile"
	                );
	        Platform.runLater(() -> {
	    ...
	    void aksiKlikListMenu() {
	        ...
	        case 0: {
	            con.loadAnchorPane(paneData, "ManageEmployee.fxml");
	        }
	            break;
	        case 1: {
	            con.loadAnchorPane(paneData, "ManageStudent.fxml");
	        }
	            break;
	      case 2: {
	            con.loadAnchorPane(paneData, "UniversityMenu.fxml");
	        }
	            break;

	        case 3: {
	            con.loadAnchorPane(paneData, "AddNewClasse.fxml");
	        }
	              break;}
	        /*
	        case 6: {
